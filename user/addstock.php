...
$feed_name = $quantity = $unit = $unit_price = '';
$errors = array();

if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Validate form fields (basic validation, adjust as per your requirements)
    ...
    if (empty($_POST['unit'])) {
        ...
        $unit = $_POST['unit'];
        if ($unit !== 'kg') {
            $errors[] = "Only 'kg' unit of measure is allowed";
        }
    }
            ...
            <div class="p-4">
                <form id="feedForm" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                        <input type="text" name="feed_name" placeholder="Feed Name" class="input-field"
                            value="<?php echo htmlspecialchars($feed_name); ?>" required>
                        <input type="number" name="quantity" placeholder="Quantity" class="input-field"
                        ...
                        <input type="number" step="0.01" name="unit_price" placeholder="Unit Price"
                            class="input-field" value="<?php echo htmlspecialchars($unit_price); ?>" required>
                    </div>
      ...
      if (closeButton) {
        ...
        if (submitButton) {
            submitButton.addEventListener('click', function () {
                // Optionally add client-side validation before submitting
